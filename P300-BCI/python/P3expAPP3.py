...
class BciApplication(BciGenericApplication):
	...
	def Initialize(self, indim, outdim):	# you should not modity this function
		...
		self.upper_cmd = socket.socket(socket.AF_INET,socket.SOCK_DGRAM)
		self.upper_cmd.bind(MYADDR)
		self.upper_cmd.setBlocking(0)
	...
	def Phases(self):	# you should not modity this function
		self.phase(name='ready', 				    next='waitcmd', 		duration=2000)
		if self.ExpMode < 2:
			self.phase(name='waitcmd', 		next='presequences', 			    duration=200)
			self.phase(name='presequences', 		next='sequences', 			duration=int(self.params['PreSequenceDuration']))
		else:
			self.phase(name='waitcmd', 		next='presequences',)
			self.phase(name='presequences', 		next='sequences', 			duration=int(self.params['PreSequenceDuration']))
		...
		self.phase(name='resdisp', 				next='waitcmd', 		duration=2500)
	...
	def Transition(self, phase):	# you should not modity this function
		if phase == 'ready':
			...
			self.stimuli['Info'].text = 'waiting'
	...
	def online_setup(self):
		textstim = VisualStimuli.Text(text = 'Ready', position  = center, anchor = 'center', color = (0,1,0), font_size = 50, on = False)
		# mz = max([int(str(x)[-2]) for x in self.stimuli.values()])
		mz = 2
		self.stimulus('Info', z=mz+1, stim = textstim)
