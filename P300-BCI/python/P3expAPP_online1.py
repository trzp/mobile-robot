...
formatter = logging.Formatter('new: %(asctime)s -- %(name)s -- %(levelname)s -- %(message)s')
...
class BciApplication(BciGenericApplication):
    ...
    def Initialize(self, indim, outdim):    # you should not modity this function
        ...
        logger.info(self.params['SubjectName'])
        logger.info('')
        ...
        if self.in_phase('resdisp') and res[0]>-1:
            if not self.ResDisplay:
                ...
                if self.trial_num > 0 and cr != self.currenttask:  #结果不正确最多三次强制执行
                    ...
                    logger.info('force result %s'%(self.currenttask))
            ...
            for c in cmds:
                if c[:4] == 'task':  #受领任务
                    ...
                    if self.targetnum == 0:
                        ...
                        logger.info('task nonsense')
                    else:
                        logger.info('task %s'%(self.currenttask))
                    ...
                    logger.info('with target num %s'%(self.targetnum))
