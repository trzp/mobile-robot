...
class BciApplication(BciGenericApplication):
	...
	def Phases(self):	# you should not modity this function
		self.phase(name='ready', 				next='presequences', 		duration=5000)
	...
	def Transition(self, phase):	# you should not modity this function
		...
		if phase == 'sequences':
			...
			# [StimulusAct(self.stimuli['cmd'+str(1+item)],(1,1,0)) for item in self.cur_codebook]
	...
	def necessary_setup(self,mode):
		...
		if mode < 2:
			...
			stim = VisualStimuli.Text(text = str, position  = (50,scrh-35), anchor = 'left', color = (1,1,1),font_name =r'C:\Windows\Fonts\times.TTF', font_size = 30, on = True)
			self.stimulus('taskcue',z=1,stim = stim)

		# result setup
		stim = VisualStimuli.Text(text = '', position  = (50,scrh-75), anchor = 'left', color = (1,1,0),font_name =r'C:\Windows\Fonts\times.TTF', font_size = 30, on = True)
	...
	def EXP_setup(self):
		...
		for i in range(36):
			...
			textstim = VisualStimuli.Text(text = CmdLst[i], position  = self.pos_sti[i], anchor = 'center', color = (1,0,1), font_size = unit, on = True)
			self.stimulus('cmd'+str(i+1),z = 1,stim = textstim)

			# 定义刺激Flsh的形状, it's all to yourself
			b = box(left=-1,right=1,bottom=-1,top=1,size = (unit,unit), position = self.pos_sti[i], sticky=True)
