...
class MobileRobot():
    def __init__(self):
        ...
        # rm car initialize
        self.rm = RmCar_x64(False)      #外部启动server
        ...
        try:
            ...
            for b in bb:
                if b in ['COMMAND_MODE','AUTO_MODE']:
                    ...
                    # else:
                    #     self.sock2bci.sendto('**targetnum5',BCI2000_ADDR)
                    ...
                    self.current_task = -1
                    # self.sock2bci.sendto('completed', BCI2000_ADDR)  # 回复bci2000动作已经完成,operator会发，这里可以不需要
    ...
    def command_scr_update(self):
        ...
        if self.assign_task >= 0:
            self.commands_cue[self.assign_task].borderon = True  # 标识当前的分类结果
        ...
        # 当前有任务
            ...
            if pos is not None:
                if self.rm.updatetask(pos):
                    self.current_task = -1
                    self.sock2bci.sendto('completed', BCI2000_ADDR)
            pass
    ...
    def mainloop(self):
        ...
        while not END:
            ...
            # print 'update image',self.UPDATE_IMAGE
